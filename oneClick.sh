...
cd ~
absoulteDir=`pwd`
mkdir -p batch/log && mkdir batch/bin
...
docker run -d -v $absoulteDir/batch/log:/batch/log -v $absoulteDir/batch/bin:/batch/bin --name batch_${version} -p 9092:9092 -e LOCATOR_HOST=gemfire -e LOCATOR_PORT=31431 -e DOWNLOAD_URL=${url}  --link gemfire_${version}:gemfire aws_batch:v1.0.${version}
